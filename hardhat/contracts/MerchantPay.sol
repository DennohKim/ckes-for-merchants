...
contract MerchantPay {
  ...
  function payForListing(
    ...
    // calculate charge -
    // note: Merchantpay only charges on rate, not on quantity for sellers cheaper experience 😊
    uint charge = deductCharge(listing.rate);

    //transfer balance to seller after charge
    IERC20(CKES).transferFrom(msg.sender, listing.seller, price - charge);
    //deduct charge
    IERC20(CKES).transferFrom(msg.sender, address(this), charge);
